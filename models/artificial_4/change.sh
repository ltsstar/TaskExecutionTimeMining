...
replace_values() {
  ...
  local new_src_location="file_location <- '../../../src/notebooks/transformed_event_logs/artificial_start_end_2_train.csv'"
  local old_nburn_pattern="nburn <- [0-9]*"
  local new_nburn="nburn <- 100000"
  local old_nsim_pattern="nsim <- [0-9]*"
  local new_nsim="nsim <- 100"
  local old_nthin_pattern="nthin <- [0-9]*"
  local new_nthin="nthin <- 100"
