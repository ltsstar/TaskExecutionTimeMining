...
replace_values() {
  ...
  local new_src_location="file_location <- '/dss/dsshome1/00/ge35xof4/TaskExecutionTimeMining/src/notebooks/transformed_event_logs/BPIC_2019_start_end_train.csv'"
  local old_nburn_pattern="nburn <- [0-9]*"
  local new_nburn="nburn <- 500"
  local old_nsim_pattern="nsim <- [0-9]*"
  local new_nsim="nsim <- 50"
  local old_nthin_pattern="nthin <- [0-9]*"
  local new_nthin="nthin <- 10"
