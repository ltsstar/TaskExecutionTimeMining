    ...
    def sample_end_time(self, case_log, start_time):
        ...
        for index, current_event in case_log.iterrows():
            ...
            if event_name in ['Match patient data', 'Wait for plate validation', 'timeout']:
                prev_finish_dt = datetime.datetime.fromtimestamp(start_time)
                seconds_in_day = get_seconds_in_day(prev_finish_dt)
                day_of_week = get_seconds_in_day(prev_finish_dt)
                finish_times[event_name] = start_time + self.sample_duration(seconds_in_day = seconds_in_day,
                                                                        resource = None,
                ...
                finish_times[event_name] = prev_finish_ts + self.sample_duration(seconds_in_day = seconds_in_day,
                                                                        resource = None,
                                                                        concept_name = event_name,
                                                                        resource_count = None,
                                               activity_count = activity_count,
                ...
                finish_times[event_name] = prev_finish_ts + \
                                            self.sample_duration(seconds_in_day = seconds_in_day,
                                                                        resource = None,
                                                                        concept_name = event_name,
                                                                        resource_count = None,
                                                                        activity_count = activity_count,
                ...
                prev_finish_ts = max(finish_times['timeout'])
                prev_finish_dt = datetime.datetime.fromtimestamp(prev_finish_ts)
                seconds_in_day = get_seconds_in_day(prev_finish_dt)
                day_of_week = get_seconds_in_day(prev_finish_dt)
