...
class StochasticLSTM(nn.Module):
    def __init__(self, 
        ...
        model_input_feat: list of features used in the model
        ...
        list_of_classes_per_cat = [cat_dict[cat_feat[0]] for cat_feat in cat_feat_model if cat_feat[0] in cat_dict]
        
        # Create embeddings for categorical features
        self.embeddings = nn.ModuleList([nn.Embedding(n_cat,
                                                      min(200, max(16, int(n_cat**0.5)))
                                                      )
                                        for n_cat in list_of_classes_per_cat])
        print("Embeddings: ", self.embeddings)
        # Compute total input size encoder
        embedding_size = sum([min(200, max(16, int(n_cat**0.5))) for n_cat in list_of_classes_per_cat])
        print("Total embedding feature size: ", embedding_size)

        self.input_size = len(num_feat_model) + embedding_size # -1 to exclude target variable
    ...
    def _input_construction(self, data):
        """
        Construct input tensor with dim: seq len x batch size x input features.
        
        Args:
            data: Dictionary with keys 'cats' and 'nums'
                - 'cats': list of tensors, one for each categorical variable
                - 'nums': list of tensors, one for each continuous variable
        
        Returns:
            x: Input tensor with shape (seq_len, batch_size, input_features)
        """
        cats = data['cats']
        nums = data['nums']
    ...
    def forward(self, input):
        ...
        """
        x = self._input_construction(data=input)
    ...
    def save(self, path: str):
        ...
        checkpoint = {
            ...
            'kwargs': {
                ...
                'model_input_feat': self.model_input_feat,
                'hidden_size': self.hidden_size,
    ...
    def load(path: str, p_fix: Optional[float] = None, device: Optional[str] = None):
        ...
        model = StochasticLSTM(**checkpoint['kwargs'])
